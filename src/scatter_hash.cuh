...
__global__ void scatter_with_hash_map(int num_edges, int num_nodes, int *input_row_ind, int *input_col_ind, int *input_csr_row, map_type *hash_map, int *total_triangle) {
...
__global__ void build_hash_map(map_type *hash_map, int *input_row_ind, int *input_col_ind, int num_edges, int num_nodes)
{
    int tx = threadIdx.x + blockIdx.x * blockDim.x;

    while (tx < num_edges) {
		long row = input_row_ind[tx];
		long col = input_col_ind[tx]; 
		long index = (row << 32) | col ; 
